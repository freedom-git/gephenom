...
if(!empty($_SESSION['manager_authority'])&&empty($_GET['category'])){
	$search_query="select t.*,u.nickname,x.picture,x.textbook_information ".
"from examination as t ".
"INNER JOIN textbook_picture AS x ON(x.id=t.title_picture) ".
"INNER JOIN user AS u ON(u.id=t.user_id) ".
"WHERE pass='0' AND price!='0' OR price='0' AND pass='0' AND youku_id!='0'";
	}else{
$search_query="select t.*,u.nickname,x.picture,x.textbook_information ".
"from examination as t ".
"INNER JOIN textbook_picture AS x ON(x.id=t.title_picture) ".
...
?>
      <div class="show_content">
               <img src="/textbook_picture/<?php echo $row['picture']?>" alt="呀！木有图！" width="180" height="227" />
	...
	echo $jiaocai[0].'—'.$jiaocai[1]?></p>
                        <p class="content_shijian">所属专辑：<a  href="/admin_check//examination/video_topics.php?sort=4&category=<?php echo $category?>&type=0&series=<?php echo $series_id?>"><?php echo $series_name?></a></p>
...
$add=0;
if(!empty($_SESSION['manager_authority'])&&empty($_GET['category'])){
$query10="SELECT series_id FROM examination WHERE pass='0'";
	}else{
$query10="SELECT series_id FROM examination WHERE category='$category' AND pass>'0'";
	}
