...
if(isset($_GET['person'])){$person=mysqli_real_escape_string($dbc,trim($_GET['person']));}else{$person=$_SESSION['id'];}
...
<div class="video_infor_container">
         <div class="infor_content">
              <div class="infor_mingzi infor_con">
                  <p><?php if($row['pass']>0){echo '<a target="_blank" href="/tutorial/intro.php?id='.$row['id'].'" class="font_1a">'.$row['heading'].'</a>';
					  }else{
						  echo $row['heading'];
						  }
					  ?></p>
              ...
              <div class="infor_zhuanti infor_con">
                  <p><a target="_blank" href="/tutorial/one.php?category=<?php echo $row['category']?>" class="font_2a"><?php
$query1="SELECT * FROM tutorial_category1 WHERE id='$row[category]' LIMIT 1";
