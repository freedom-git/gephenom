...
$query0="SELECT * FROM buy_tutorial WHERE tutorial_id='$id' AND user_id='$_SESSION[session_user_id]' AND times<'10' LIMIT 1";
$result0=mysqli_query($dbc,$query0) or die('出错');
$buyornot=mysqli_num_rows($result0);
$row0=mysqli_fetch_array($result0);
if(isset($_POST['submit'])||!empty($_COOKIE[$row['id'].$_SESSION['session_user_id'].'t'])){$buyornot=1;}
...
//插入购买信息并划分钱款
      $query4="insert into buy_tutorial (user_id,tutorial_id) values ('$_SESSION[session_user_id]','$id')";//插入购买信息
	  ...
	  $query_trade1="insert into trade (user_id,how_much,trade_no,balance,type) values ('$_SESSION[session_user_id]','$row[price]','$id','$balance1','4')";//写入购买用户余额
      $result_trade1=mysqli_query($dbc,$query_trade1) or die('错误1');
	  $how_much=$row['price']/2;
	  $query_trade2="insert into trade (user_id,how_much,trade_no,balance,type,buyer) values ('$row[user_id]','$how_much','$id','$balance2','3','$_SESSION[session_user_id]')";//写入上传用户余额
...
$query22="UPDATE buy_tutorial SET grade='$grade',criticism='$criticism',criticism_date=now() WHERE tutorial_id='$id' AND user_id='$_SESSION[session_user_id]'  AND times<'10' LIMIT 1";
...
//写入评分
$query1="SELECT * FROM buy_tutorial WHERE tutorial_id='$id' AND user_id='$_SESSION[session_user_id]' AND times<='10' LIMIT 1";
...
<?php
//控制播放次数
if(!isset($_POST['submit'])&&empty($_COOKIE[$row['id'].$_SESSION['session_user_id'].'t'])){
$times=$row0['times']+1;
$queryadd1="UPDATE buy_tutorial SET times='$times' WHERE tutorial_id='$id' AND user_id='$_SESSION[session_user_id]' AND times<'10' LIMIT 1";
$resultadd1=mysqli_query($dbc,$queryadd1) or die('错误');
setcookie($row['id'].$_SESSION['session_user_id'].'t','times',time()+7200);
}
?>
<html>
<body>
